...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 40 patterns into a single 130-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(40);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[129]?.kind).toBe("pattern");
  });
