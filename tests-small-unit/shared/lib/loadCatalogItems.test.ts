...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 17 patterns into a single 107-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(17);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[106]?.kind).toBe("pattern");
  });
