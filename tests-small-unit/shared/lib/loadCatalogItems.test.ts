...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 10 patterns into a single 100-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(10);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[99]?.kind).toBe("pattern");
  });
