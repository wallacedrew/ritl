...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 39 patterns into a single 129-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(39);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[128]?.kind).toBe("pattern");
  });
