...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 27 patterns into a single 117-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(27);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[116]?.kind).toBe("pattern");
  });
