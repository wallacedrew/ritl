...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 11 patterns into a single 101-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(11);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[100]?.kind).toBe("pattern");
  });
