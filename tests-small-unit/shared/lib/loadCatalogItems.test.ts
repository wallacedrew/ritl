...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 5 patterns into a single 95-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(5);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[94]?.kind).toBe("pattern");
  });
