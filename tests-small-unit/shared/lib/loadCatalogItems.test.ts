...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 24 patterns into a single 114-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(24);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[113]?.kind).toBe("pattern");
  });
