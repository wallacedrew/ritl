...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 35 patterns into a single 125-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(35);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[124]?.kind).toBe("pattern");
  });
