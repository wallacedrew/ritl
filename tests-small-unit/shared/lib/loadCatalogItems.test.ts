...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 47 patterns into a single 137-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(47);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[136]?.kind).toBe("pattern");
  });
