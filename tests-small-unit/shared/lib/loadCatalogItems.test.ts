...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 43 patterns into a single 133-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(43);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[132]?.kind).toBe("pattern");
  });
