...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 6 patterns into a single 96-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(6);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[95]?.kind).toBe("pattern");
  });
