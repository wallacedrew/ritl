...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 12 patterns into a single 102-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(12);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[101]?.kind).toBe("pattern");
  });
