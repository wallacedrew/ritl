...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 30 patterns into a single 120-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(30);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[119]?.kind).toBe("pattern");
  });
