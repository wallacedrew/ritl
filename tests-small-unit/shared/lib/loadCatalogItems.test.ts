...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 4 patterns into a single 94-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(4);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[93]?.kind).toBe("pattern");
  });
