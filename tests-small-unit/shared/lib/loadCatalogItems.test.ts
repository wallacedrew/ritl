...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 46 patterns into a single 136-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(46);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[135]?.kind).toBe("pattern");
  });
