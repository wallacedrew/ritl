...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 36 patterns into a single 126-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(36);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[125]?.kind).toBe("pattern");
  });
