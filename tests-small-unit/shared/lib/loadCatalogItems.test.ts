...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 8 patterns into a single 98-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(8);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[97]?.kind).toBe("pattern");
  });
