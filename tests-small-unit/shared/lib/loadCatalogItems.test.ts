...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 19 patterns into a single 109-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(19);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[108]?.kind).toBe("pattern");
  });
