...
describe("loadCatalogItems", () => {
  it("combines 24 smells, 66 refactorings, and 31 patterns into a single 121-item catalog", () => {
    ...
    expect(items.filter((i) => i.kind === "pattern")).toHaveLength(31);
  });
  ...
  it("lists refactorings, then smells, then patterns", () => {
    ...
    expect(items[120]?.kind).toBe("pattern");
  });
