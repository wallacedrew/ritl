...
/**
 * Glossary keys eligible for the human-side marking lint.
 *
 * Includes the cost-to-human vocabulary, the cross-cutting design
 * principles, and the failure-mode terms that originate in cognitive
 * psychology and apply to readers as well as agents. Excludes terms
 * that only make sense on the agent side (context window, token cost,
 * RAG, chain-of-thought, "the agent", etc.).
 */
  ...
  for (const [key, keyEvents] of eventsByKey) {
    ...
    if (markedEvents.length > 1) {
      hits.push({
        ...
        key,
        reason: `over-marked: ${markedEvents.length} {{${key}}} markers on the human side; the rule is first-occurrence per entry only`,
      });
      continue;
    }
    ...
    if (markedEvents.length === 0) {
      ...
      hits.push({
        entry: entry.name.toString(),
        key,
        reason: `missing marker: '${key}' appears unmarked in human.${firstField} (and possibly elsewhere); entries that opt in must mark every lintable key on its first occurrence`,
      });
      continue;
    }
    ...
    if (firstEvent.kind !== "marked") {
      ...
      hits.push({
        entry: entry.name.toString(),
        key,
        reason: `marker out of position: unmarked '${key}' in human.${firstUnmarkedField} precedes the {{${key}}} marker in human.${markerField}`,
      });
    }
  }
  return hits;
...
describe("opted-in entries mark every lintable human-side glossary key on its first occurrence", () => {
  it("emits no hits for entries that have at least one {{glossary-key}} marker on the human side", () => {
...
describe("human-side LINTABLE_KEYS allow-list is in sync with the glossary", () => {
