...
### Universal invariant — no domain concept without a value object

This rule is non-negotiable and applies to every slice from Slice 1 onward:

> **No domain concept that crosses ≥2 layers, ≥2 modules, or ≥2 function signatures lives as a bare primitive. Every such concept goes through a value object (class with validation at construction, domain methods, no setters) or an equivalently strict nominal type. There are zero exceptions.**

Domain concepts include: identifiers (slug, id, key, handle), addressable resources (URL, href, path), money or quantities (`cents: number`, `quantityMl: number`), enumerated kinds that ride as strings (`kind: "smell" | "refactoring"`), validated free-text fields with invariants (skill description ≤1,536 chars and no `: ` colon-space, etc.), and any number that carries units.

The first time a slice introduces a domain concept that doesn't yet have a value object, the slice is responsible for:

1. Defining the value object class under the owning feature folder (`src/<feature>/lib/<Concept>.ts`), sized to _only_ what this slice needs. Don't pre-declare methods for behaviors no slice has asked for yet — extend the class in the slice that actually needs the new method.
2. Putting validation in the constructor (or in a static `from()` factory). Bare construction is banned; consumers always go through the validator. The validator is the one place that knows the rules — no scattered `if (!regex.test(slug)) throw…` at call sites.
3. Adding microtests for the constructor under `tests-small-unit/<feature>/lib/<Concept>.test.ts` — at minimum: happy path, every rejection rule, equality semantics.
4. Replacing the bare primitive in every call site this slice touches. Don't leave a stray `slug: string` in a sibling function — the type system has to actively enforce the constraint, not just suggest it. A leftover bare primitive is a hole the smell creeps back through.

Subsequent slices that touch the same concept reuse the existing value object. Slices that introduce new concepts repeat this dance for each new one. **Value objects grow incrementally; the codebase never carries a class for a concept no slice has shipped.**

The Design Principles section's _"Value Objects"_ bullet below answers _what shape the class takes_; this section answers _when and how to introduce one in a slice_. Both apply.
...
- **Value Objects — wrap any primitive that carries domain meaning across ≥2 call sites.** When a `string`, `number`, or `boolean` parameter's _name_ says more than its _type_ does — `slug: string`, `id: string`, `email: string`, `cents: number`, `kind: "smell" | "refactoring"` — and that same shape appears in a second function signature, extract a value object **before** the second usage. The value object lives at `src/<feature>/lib/<Concept>.ts`, exports a class, validates at construction (no setters, no bare construction), and offers domain methods (`Slug.from(name).toHref()`, `Money.cents(500).plus(tax)`).
  - **Trigger:** the same domain-named primitive appears in two function signatures, two interface fields, or one signature plus one return type. _Don't_ wait for the third occurrence — that's already two too late.
  - **Pitfall:** don't pre-wrap one-shot strings used inside a single function — the rule is "rule of two," not "every primitive." Equally: a TypeScript `interface` or `type` alias over a plain object is _not_ a value object. Without a constructor that validates and methods that own behavior, you've named a record, not wrapped a primitive — the smell lives on inside the record's fields.
  - **How to apply:** see the parallel kickoff invariant _"no domain concept without a value object"_ in Project Kickoff Sequence above for _when_ to introduce one in a slice; this rule is about _what shape it takes_ when you do.
- **Default to OOP outside view files** — in any feature's `lib/` or `adapters/` (registries, services with state + behaviour, plus value objects per the rule above), reach for classes that compose data + methods. View-layer consumers stay functional. Plain functions remain the right tool for pure transforms and parsers. Component-framework idioms (functional components, hooks) keep using functions — that's the framework's shape, not a contradiction.
